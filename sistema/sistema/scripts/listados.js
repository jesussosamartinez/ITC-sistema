$.post("../ajax/talleres.php?op=listar", function (data, status) {
  ...
  for (var i = 0; i < data.aaData.length; i++) {
    var $option =
      "<option value=" +
      data.aaData[i].Nombre +
      ">" +
      data.aaData[i].Nombre +
      " [" +
      data.aaData[i].Horario +
      "]</option>";
...
$("#formsestadisticas").on("submit", function (e) {
  ...
  $.post(
    "../ajax/alumnotaller.php?op=estadisticas",
    {
      periodo: periodo,
    },
    function (data) {
    ...
    var frm = document.getElementById("buscador");
    var tbl = document.getElementById("tblestadisticas");
    if (
