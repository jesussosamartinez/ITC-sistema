  ...
  $("#actualizardatos").submit(function( event ) {
	...
	$("#añadirdatos").submit(function( event ) {
		...
		var parametros = $(this).serialize();
			$.ajax({
				   ...
				   alertify.alert('REGISTRO DE ALUMNO',datos, function(){ 
					...
					registrarEnBitacora(UsuarioRegistrado, "Registro de usuario correcto", function(success) {
						if (success) {
	   ...
	   function registrarEnBitacora(usuario, estatus, callback) {
		$.post("../ajax/Bitacora.php?op=RegistroUsuario", {
			"Movimiento": "Registro de usuario",
			"Usuario": usuario,
			"Estatus": estatus
		}, function(data) {
			callback(data != "null");
...
function CopiarNombre(){
	UsuarioRegistrado= $("#ncontrolu").val();
